...
// Utterleaf modification: bound session context copies and reject unavailable dictionary owners.
...
static void latinime_initDicTraverseSession(JNIEnv *env, jclass clazz, jlong traverseSession,
    ...
    if (env->ExceptionCheck() || previousWordLength < 0
            || previousWordLength > MAX_WORD_LENGTH) return;
    DicTraverseSession *ts = reinterpret_cast<DicTraverseSession *>(traverseSession);
    if (!ts) {
        return;
    }
    Dictionary *dict = reinterpret_cast<Dictionary *>(dictionary);
    if (!dict || !dict->isStorageAvailable()) return;
    if (!previousWord) {
        if (previousWordLength != 0) return;
        NgramContext emptyNgramContext;
        ts->init(dict, &emptyNgramContext, 0 /* suggestOptions */);
        return;
    }
    const jsize capacity = env->GetArrayLength(previousWord);
    if (env->ExceptionCheck() || previousWordLength > capacity) return;
    int prevWord[MAX_WORD_LENGTH] = {};
    env->GetIntArrayRegion(previousWord, 0, previousWordLength, prevWord);
    if (env->ExceptionCheck()
            || !JniDataUtils::isValidWordCodePoints(prevWord, previousWordLength)) return;
