...
// Utterleaf modification: preserve negative sentinels without indexing the Unicode table.
...
class CharUtils {
 public:
    ...
    static AK_FORCE_INLINE int toBaseCodePoint(int c) {
        // Utterleaf: native sentinel values are not Unicode table indexes. Preserve them.
        if (c >= 0 && c < BASE_CHARS_SIZE) {
