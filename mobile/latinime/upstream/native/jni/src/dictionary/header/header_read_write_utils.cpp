...
// Utterleaf modification: bound ASCII numeric metadata parsing without signed overflow.
#include <climits>
...
/* static */ int HeaderReadWriteUtils::readIntAttributeValueInner(
    ...
    if (it != headerAttributes->end()) {
        // Accumulate negatively so INT_MIN remains representable throughout parsing.
        int value = 0;
        const bool isNegative = !it->second.empty() && it->second[0] == '-';
        const int limit = isNegative ? INT_MIN : -INT_MAX;
        for (size_t i = isNegative ? 1 : 0; i < it->second.size(); ++i) {
            const int codePoint = it->second[i];
            if (codePoint < '0' || codePoint > '9') return defaultValue;
            const int digit = codePoint - '0';
            if (value < (limit + digit) / 10) return defaultValue;
