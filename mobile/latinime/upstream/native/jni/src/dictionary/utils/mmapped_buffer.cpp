...
// Utterleaf modification: validate mapping ranges against the same open descriptor.
...
/* static */ MmappedBuffer::MmappedBufferPtr MmappedBuffer::openBuffer(
    ...
    if (bufferOffset < 0 || bufferSize <= 0) return nullptr;
    ...
    const int64_t end = static_cast<int64_t>(bufferOffset) + bufferSize;
    const long pageSize = sysconf(_SC_PAGESIZE);
    if (fstat(mmapFd, &fileStat) != 0 || !S_ISREG(fileStat.st_mode)
            || fileStat.st_size < 0 || end > fileStat.st_size
            || pageSize <= 0 || pageSize > INT_MAX) {
        close(mmapFd);
        return nullptr;
    }
    const int pagesize = static_cast<int>(pageSize);
    const int offset = bufferOffset % pagesize;
    const int alignedOffset = bufferOffset - offset;
    const int64_t wideAlignedSize = static_cast<int64_t>(bufferSize) + offset;
    if (wideAlignedSize > INT_MAX) {
        close(mmapFd);
        return nullptr;
    }
    const int alignedSize = static_cast<int>(wideAlignedSize);
    ...
    if (!buffer) {
        ...
        munmap(mmappedBuffer, alignedSize);
        close(mmapFd);
        return nullptr;
    }
