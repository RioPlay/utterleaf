...
// Utterleaf modification: absent or incomplete decoder policies return unavailable safely.
...
void Suggest::getSuggestions(ProximityInfo *pInfo, void *traverseSession,
    ...
    // The standalone AOSP foundation has no registered gesture policy. An absent or
    // incomplete policy means suggestions are unavailable, never a substitute decoder.
    if (!TRAVERSAL || !SCORING || !WEIGHTING) return;
    PROF_INIT;
    PROF_TIMER_START(0);
