...
// Utterleaf modification: generation-bound persistence and unavailable-state quarantine.
...
bool Dictionary::flush(const char *const filePath) {
    if (!mStorageAvailable) return false;
    StorageTransaction transaction(filePath, &mStorageGeneration, &mStorageFault);
    if (!transaction.admitted()) return false;
    TimeKeeper::setCurrentTime();
    const bool result = mDictionaryStructureWithBufferPolicy->flush(filePath);
    if (!result) mStorageAvailable = false;
#ifdef UTTERLEAF_STORAGE_TESTING
    if (result && mStorageFault.stop(7)) gStorageFailNextNativeOpen = true;
#endif
    return result;
}
...
bool Dictionary::flushWithGC(const char *const filePath) {
    ...
    if (!transaction.admitted()) return false;
    TimeKeeper::setCurrentTime();
    const bool result = mDictionaryStructureWithBufferPolicy->flushWithGC(filePath);
    if (!result) mStorageAvailable = false;
#ifdef UTTERLEAF_STORAGE_TESTING
    if (result && mStorageFault.stop(7)) gStorageFailNextNativeOpen = true;
#endif
