...
// Utterleaf modification: retain storage generations and gate unavailable dictionary owners.
...
class Dictionary {
 public:
    ...
    // Utterleaf: an interrupted/failed admitted write may leave GC's private state changed.
    bool isStorageAvailable() const { return mStorageAvailable; }
    void markStorageUnavailable() { mStorageAvailable = false; }
    void setStorageGeneration(std::shared_ptr<StorageGeneration> generation) {
        mStorageGeneration = std::move(generation);
    }
#ifdef UTTERLEAF_STORAGE_TESTING
    StorageFault &storageFaultForTesting() { return mStorageFault; }
#endif
 ...
 private:
    ...
    std::shared_ptr<StorageGeneration> mStorageGeneration;
    StorageFault mStorageFault;
    bool mStorageAvailable = true;
