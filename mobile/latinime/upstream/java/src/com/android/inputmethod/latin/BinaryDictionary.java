 ...
 * Utterleaf: overlapping operations from another thread return unavailable values;
 * native dictionary and traversal buffers are exclusively owned until return.
 * Close rejects new operations immediately and defers native disposal to that owner.
 */
// Utterleaf modification: native operations have exclusive, nonwaiting lifetime ownership.
public final class BinaryDictionary extends Dictionary {
    ...
    private final NativeOperationGate mNativeOperations =
            new NativeOperationGate(this::disposeNativeResources, this::disposeTraverseSessions);
    private final long mDictSize;
    private final String mDictFilePath;
    private final boolean mUseFullEditDistance;
    private final boolean mIsUpdatable;
    private boolean mHasUpdated;
    private volatile boolean mStorageAvailable;
    ...
    public BinaryDictionary(final String filename, final boolean useFullEditDistance,
        ...
        mStorageAvailable = mNativeDict != 0;
    }
    ...
    private static native void markStorageUnavailableNative(long dict);
    private static native boolean isStorageAvailableNative(long dict);
    ...
    public boolean isCorrupted() {
        final NativeOperationGate.Lease operation = mNativeOperations.tryAcquire();
        if (operation == null) {
            return false;
        }
        try {
            ...
            Log.e(TAG, "Dictionary is unavailable; preserving its files.");
            return true;
        } finally {
            operation.close();
        }
    ...
    public ArrayList<SuggestedWordInfo> getSuggestions(final ComposedData composedData,
        ...
        try {
            // Validate only after admission: retirement either rejects this request or
            // observes our active lease and disposes its sessions when we return.
            if (!composedData.isRequestCurrent() || !isValidDictionary()) {
                return null;
            }
            ...
            getSuggestionsNative(mNativeDict, proximityInfoHandle,
                    session.getSession(), inputPointers.getXCoordinates(),
    ...
    public boolean isValidDictionary() {
        // Busy decoding is distinct from unavailable storage. Native policy entrypoints
        // remain authoritative; this flag is updated under the existing write leases.
        return mNativeOperations.isOpen() && mNativeDict != 0 && mStorageAvailable;
    }
    ...
    public GetNextWordPropertyResult getNextWordProperty(final int token) {
        final NativeOperationGate.Lease operation = mNativeOperations.tryAcquire();
        if (operation == null) {
            return new GetNextWordPropertyResult(null, 0);
        }
        try {
    ...
    public void updateEntriesForInputEvents(final WordInputEventForPersonalization[] inputEvents) {
        // Utterleaf: legacy bulk personalization is intentionally unavailable. A reviewed
        // explicit import contract must not inherit passive-learning/event validity semantics.
        throw new UnsupportedOperationException("Legacy bulk personalization is unavailable");
    }
    ...
    private boolean reopen() {
        ...
        // Utterleaf: validate a replacement owner before releasing the current resources.
        final long replacement = openNative(dictFile.getAbsolutePath(), 0 /* startOffset */,
                dictFile.length(), mIsUpdatable);
        if (replacement == 0) {
            markStorageUnavailableNative(mNativeDict);
            mStorageAvailable = false;
            return false;
        }
        disposeNativeResources();
        mNativeDict = replacement;
        ...
        try {
            ...
            if (mHasUpdated) {
                if (!flushNative(mNativeDict, mDictFilePath)) {
                    mStorageAvailable = isStorageAvailableNative(mNativeDict);
                    return false;
                }
                return reopen();
            }
    ...
    public boolean migrateTo(final int newFormatVersion) {
        // Utterleaf: automatic migration is temporarily unavailable until a recoverable,
        // verified replacement transaction exists. Preserve the current owner and all files.
        // The private native candidate-copy routine is not a production replacement operation.
        return false;
    }
    ...
    /** Holds real native ownership for deterministic lifecycle tests, without exposing pointers. */
    @UsedForTesting
    public boolean runWithNativeOperationForTesting(final Runnable operation) {
        final NativeOperationGate.Lease lease = mNativeOperations.tryAcquire();
        if (lease == null) {
            return false;
        }
        try {
    ...
    /** Retire decoder caches without unloading the dictionary or waiting for native decoding. */
    @Override
    public void clearSession() {
        mNativeOperations.requestMaintenance();
    }

    @Override
    public void close() {
        mStorageAvailable = false;
        mNativeOperations.close();
    }

    // Called only with exclusive operation ownership, or after terminal quiescence.
    private void disposeNativeResources() {
        try {
            disposeTraverseSessions();
        } finally {
            closeNativeDictionary();
        }
    }

    private void disposeTraverseSessions() {
        final ArrayList<DicTraverseSession> retiredSessions = new ArrayList<>();
        for (int index = 0; index < mDicTraverseSessions.size(); ++index) {
            retiredSessions.add(mDicTraverseSessions.valueAt(index));
        }
        // Detach before disposal so an exceptional close cannot leave old buffers reusable.
        mDicTraverseSessions.clear();
        Throwable failure = null;
        for (final DicTraverseSession session : retiredSessions) {
            try {
    ...
    private void closeNativeDictionary() {
        if (mNativeDict != 0) {
            final long retiredDictionary = mNativeDict;
            mNativeDict = 0;
            closeNative(retiredDictionary);
        }
    }
