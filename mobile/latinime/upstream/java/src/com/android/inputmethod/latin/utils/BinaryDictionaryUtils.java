...
public final class BinaryDictionaryUtils {
    ...
    public static float calcNormalizedScore(final String before, final String after,
            final int score) {
        // Utterleaf resource policy mirrors the authoritative native checks: at most 4096
        // codepoints per input and 1,048,576 DP cells. Unsupported work is unavailable (0),
        // never truncated. A supplementary codepoint occupies two UTF-16 code units.
        if (before == null || after == null || score <= 0
                || before.length() > 8192 || after.length() > 8192) return 0.0f;
        final int beforeLength = before.codePointCount(0, before.length());
        final int afterLength = after.codePointCount(0, after.length());
        if (beforeLength > 4096 || afterLength > 4096
                || (beforeLength + 1) > 1048576 / (afterLength + 1)) return 0.0f;
