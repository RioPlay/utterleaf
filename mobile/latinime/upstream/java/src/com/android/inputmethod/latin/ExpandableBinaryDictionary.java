...
// Utterleaf modification: retire transient native sessions and tolerate unavailable debug headers.
...
abstract public class ExpandableBinaryDictionary extends Dictionary {
     ...
     */
    private volatile BinaryDictionary mBinaryDictionary;
    ...
    // Utterleaf: failed automatic loads preserve their files and remain unavailable.
    private volatile boolean mAutomaticReloadBlocked;
    ...
    public void clearSession() {
        // Publish retirement immediately, before a new-current reader can reuse old sessions.
        // The binary gate safely handles concurrent close/replacement without this worker lock.
        final BinaryDictionary dictionary = mBinaryDictionary;
        if (dictionary != null) dictionary.clearSession();
    }
    ...
    private void retireCorruptedDictionary(final BinaryDictionary corruptedDictionary) {
        // Called under the read lock: stop automatic reload before queuing exclusive retirement.
        mAutomaticReloadBlocked = true;
        asyncExecuteTaskWithWriteLock(new Runnable() {
            @Override
            public void run() {
                if (mBinaryDictionary == corruptedDictionary) closeBinaryDictionary();
            }
        });
    }
    ...
    public void clear() {
        asyncExecuteTaskWithWriteLock(new Runnable() {
            @Override
            public void run() {
                // Explicit clear is distinct from automatic recovery/rebuild failures.
                mAutomaticReloadBlocked = false;
    ...
    public void runGCIfRequired(final boolean mindsBlockByGC) {
        asyncExecuteTaskWithWriteLock(new Runnable() {
            @Override
            public void run() {
                if (mAutomaticReloadBlocked || getBinaryDictionary() == null) {
                    return;
                }
    ...
    public void removeUnigramEntryDynamically(final String word) {
        ...
        asyncExecuteTaskWithWriteLock(new Runnable() {
            ...
            public void run() {
                ...
                if (mAutomaticReloadBlocked || binaryDictionary == null) {
                    return;
                }
                runGCIfRequiredLocked(true /* mindsBlockByGC */);
    ...
    public void updateEntriesForInputEvents(
        ...
        // Utterleaf: reject synchronously, before reload/worker scheduling or completion callbacks.
    ...
    public ArrayList<SuggestedWordInfo> getSuggestions(final ComposedData composedData,
        ...
        try {
            ...
            if (lockAcquired) {
                if (mAutomaticReloadBlocked || mBinaryDictionary == null) {
                    return null;
                }
                ...
                if (mBinaryDictionary.isCorrupted()) {
                    Log.w(TAG, "Dictionary is corrupted; preserving existing files.");
                    retireCorruptedDictionary(mBinaryDictionary);
                    return null;
                }
                return suggestions;
            }
    ...
    protected boolean isInDictionaryLocked(final String word) {
        if (mAutomaticReloadBlocked || mBinaryDictionary == null) return false;
    ...
    void loadBinaryDictionaryLocked() {
        ...
        if (mBinaryDictionary.isValidDictionary()
            ...
            if (!mBinaryDictionary.migrateTo(DICTIONARY_FORMAT_VERSION)) {
                // Utterleaf: never regenerate/delete a legacy dictionary after migration failure.
                // Null also prevents asyncReloadDictionary's format-mismatch recreation branch.
                Log.w(TAG, "Dictionary migration is unavailable; preserving existing files.");
                mAutomaticReloadBlocked = true;
                closeBinaryDictionary();
            }
        }
    ...
    private boolean isReloadRequired() {
        return !mAutomaticReloadBlocked && (mBinaryDictionary == null || mNeedsToRecreate);
    }
    ...
    private void asyncReloadDictionary() {
        ...
        asyncExecuteTaskWithWriteLock(new Runnable() {
            ...
            public void run() {
                try {
                    if (mAutomaticReloadBlocked) return;
                    if (!dictFile.exists()) {
                        createNewDictionaryLocked();
                    } else if (isNeededToRecreate()) {
                        // A rebuild signal does not authorize deleting an existing dictionary.
                        mAutomaticReloadBlocked = true;
                        closeBinaryDictionary();
                        ...
                        if (binaryDictionary != null && !(isValidDictionaryLocked()
                            ...
                            // Invalid/unsupported input remains on disk for explicit recovery.
                            mAutomaticReloadBlocked = true;
                            closeBinaryDictionary();
                        }
                    }
