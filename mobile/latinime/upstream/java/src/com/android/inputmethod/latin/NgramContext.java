...
public class NgramContext {
    ...
    // Utterleaf: detach mutable caller-owned arrays and CharSequences before worker dispatch.
    public NgramContext snapshot() {
        final WordInfo[] words = new WordInfo[mPrevWordsCount];
        for (int i = 0; i < words.length; ++i) {
            final WordInfo word = mPrevWordsInfo[i];
            words[i] = word == null ? WordInfo.EMPTY_WORD_INFO
                    : word.mIsBeginningOfSentence ? WordInfo.BEGINNING_OF_SENTENCE_WORD_INFO
                    : new WordInfo(word.mWord == null ? null : word.mWord.toString());
        }
        return new NgramContext(mMaxPrevWordCount, words);
    }
