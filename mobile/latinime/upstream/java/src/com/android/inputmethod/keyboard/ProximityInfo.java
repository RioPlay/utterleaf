...
// Utterleaf modification: native operation leases retain proximity resources through decoder use.
...
public class ProximityInfo {
    ...
    private volatile long mNativeProximityInfo;
    // Utterleaf: each native consumer retains this owner through its operation lease.
    private final NativeOperationGate mNativeOperations =
            new NativeOperationGate(this::disposeNativeProximityInfo);
    static {
    ...
    /** Acquire before reading the native handle; release only after all JNI use returns. */
    public NativeOperationGate.Lease acquireNativeOperation() {
        return mNativeOperations.tryAcquire();
    }
    ...
    private void disposeNativeProximityInfo() {
        final long retiredProximityInfo = mNativeProximityInfo;
        mNativeProximityInfo = 0;
        if (retiredProximityInfo != 0) {
            releaseProximityInfoNative(retiredProximityInfo);
        }
    }
