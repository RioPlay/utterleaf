...
import org.utterleaf.keyboard.SuggestionComposerSnapshot;
...
public final class InputLogic {
    ...
    private boolean mInputStateRetired = true;

    public boolean isInputStateRetired() {
        return mInputStateRetired;
    }
    ...
    public void startInput(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        mRecapitalizeStatus.reset(); // Do not perform recapitalize until the cursor is moved once
    ...
    public void onSubtypeChanged(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        if (mLatinIME.mEditorSession.capture() == null) {
            ...
            // finish composition on an editor whose lifetime has already ended.
            clearLocalState();
            return;
        }
        ...
        finishInput();
        // The editor remains the same across a subtype switch. Rebuild its prefix cache
        // using the current tracked cursor, never EditorInfo's potentially stale initial one.
    ...
    /**
     * Retire local input even when no input view was started. The owner must first
     * invalidate its EditorSession so already-running decoder results stay rejected.
     * This deliberately makes no call to the editor or framework composition state.
     */
    public void retireInput() {
        mInputLogicHandler.reset();
        clearLocalState();
    }

    /** Release session-owned state without making any call to an editor. */
    private void clearLocalState() {
        mInputStateRetired = true;
        resetComposingState(true /* alsoResetLastComposedWord */);
        mWordComposer.setCapitalizedModeAtStartComposingTime(WordComposer.CAPS_MODE_OFF);
        mSuggestedWords = SuggestedWords.getEmptyInstance();
        mEnteredText = null;
        mWordBeingCorrectedByCursor = null;
        mRecapitalizeStatus.reset();
        mCurrentlyPressedHardwareKeys.clear();
        ...
        mLastKeyTime = 0;
        cancelDoubleSpacePeriodCountdown();
        mConnection.clearLocalState();
    }

    /** Shut down deferred decoding without finishing composition on a possibly stale editor. */
    public void destroy() {
    ...
    public boolean onUpdateSelection(final int oldSelStart, final int oldSelEnd,
        ...
        // A finished framework session can still deliver selection updates while its decor
        // remains visible. Do not reacquire a connection or repopulate retired local caches.
        // onStartInput establishes an active token even without a software input view.
        if (mLatinIME.mEditorSession.capture() == null) return false;
        if (mConnection.isBelatedExpectedUpdate(oldSelStart, newSelStart, oldSelEnd, newSelEnd)) {
            return false;
        }
        // Utterleaf: an unexpected selection is a new request context in the same editor.
        // Retire the old caret's worker/UI results before any composing or connection changes.
        if (mInputLogicHandler == InputLogicHandler.NULL_HANDLER) {
            mLatinIME.mEditorSession.renewIfActive();
            mLatinIME.mHandler.cancelPendingSuggestionResults();
        } else {
        ...
        clearAutoCorrectionIndicator();
        // This helper only retires local caches/views. Host IPC below holds no native gate.
        mLatinIME.clearEditorTransientState();
    ...
    public void onCancelBatchInput(final LatinIME.UIHandler handler) {
        ...
        // Cancel only this gesture. The previous committed word remains available for undo.
    ...
    public void performUpdateSuggestionStripSync(final SettingsValues settingsValues,
        ...
        final SuggestedWords previousSuggestions = mSuggestedWords;
                ...
                new OnGetSuggestedWordsCallback() {
                    ...
                    public void onGetSuggestedWords(final SuggestedWords suggestedWords) {
                        ...
                        if (suggestedWords.size() > 1 || typedWordString.length() <= 1) {
                            ...
                            holder.set(retrieveOlderSuggestions(typedWordInfo, previousSuggestions));
                        }
                    }
                }
        );
    ...
    void doShowSuggestionsAndClearAutoCorrectionIndicator(final SuggestedWords suggestedWords) {
        // The worker only enqueues. Indicator state belongs to the owner thread, and must
        // not be changed by a result whose editor expires before final UI delivery.
        mLatinIME.mHandler.showSuggestionStrip(suggestedWords, true);
    }

    public void clearAutoCorrectionIndicator() {
        mIsAutoCorrectionIndicatorOn = false;
    }
    ...
    private void commitChosenWord(final SettingsValues settingsValues, final String chosenWord,
        ...
        if (DebugFlags.DEBUG_ENABLED) {
            startTimeMillis = System.currentTimeMillis();
            // Utterleaf: never log the chosen text, including in debug builds.
        }
        final SuggestedWords suggestedWords = mSuggestedWords;
        // TODO: Locale should be determined based on context and the text given.
    ...
    // Utterleaf: capture editor/composer state before dispatch; decoding never reads the live IME.
    public SuggestionDecoder captureSuggestionRequest(final SettingsValues settingsValues,
            final Keyboard keyboard, final int keyboardShiftMode, final int inputStyle,
            final int sequenceNumber) {
        mWordComposer.adviseCapitalizedModeBeforeFetchingSuggestions(
                getActualCapsMode(settingsValues, keyboardShiftMode));
        final org.utterleaf.keyboard.EditorSession session = mLatinIME.mEditorSession;
