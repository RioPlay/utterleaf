...
// Java WorkLease serializes inference and model replacement. Native work_id is a
// second gate: reset starts a new generation, and in-flight decode from an older
// generation aborts even if a later reset cleared a boolean cancel flag.
static std::atomic<int64_t> work_id{0};
static std::atomic<int64_t> cancelled_id{0};
static bool stale(int64_t id) {
    return id <= 0 || work_id.load() != id || cancelled_id.load() >= id;
}
static bool abort_decode(void *userdata) {
    return stale(*static_cast<const int64_t *>(userdata));
}
static void quiet_log(enum ggml_log_level, const char *, void *) {}
extern "C" JNIEXPORT void JNICALL
Java_org_utterleaf_voice_NativeEngine_reset(JNIEnv *, jobject) { work_id.fetch_add(1); }
extern "C" JNIEXPORT void JNICALL
Java_org_utterleaf_voice_NativeEngine_cancel(JNIEnv *, jobject) { cancelled_id.store(work_id.load()); }

extern "C" JNIEXPORT jbyteArray JNICALL
Java_org_utterleaf_voice_NativeEngine_decode(JNIEnv *env, jobject, jstring model, jfloatArray input) {
    try {
        int64_t id = work_id.load();
        const auto count = env->GetArrayLength(input);
        if (count <= 0 || count > 16000 * 120 || stale(id)) return nullptr;
        ...
        if (!ctx || stale(id)) return nullptr;
        std::vector<float> samples(count);
        env->GetFloatArrayRegion(input, 0, count, samples.data());
        if (env->ExceptionCheck() || stale(id)) {
        ...
        p.abort_callback_user_data = &id;
        const int result = whisper_full(ctx.get(), p, samples.data(), count);
        std::fill(samples.begin(), samples.end(), 0.0f);
        if (result != 0 || stale(id)) return nullptr;
        std::string text;
        ...
        if (stale(id)) {
            if (bytes) {
                ...
                std::vector<jbyte> zeros(n, 0);
                env->SetByteArrayRegion(bytes, 0, n, zeros.data());
            }
            return nullptr;
        }
        return bytes;
