...
| It stays private | Idle microphone state, no retained recording history, verified cleanup of explicitly authorized temporary capture files, debug logs free of dictated text, model-only network access, and an offline run after model installation. Check dependency behavior as well as app code. |
